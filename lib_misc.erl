-module(lib_misc).
-export([pythag/1, perms/1, tuple_to_list/1, fac/1, on_exit/2, keep_alive/2, file_size_and_type/1, ls/1]).
...
tuple_to_list(T) -> tuple_accumulator(T, [], 1).

tuple_accumulator(T, L, N) when N > tuple_size(T) -> lists:reverse(L);
tuple_accumulator(T, L, N) -> tuple_accumulator(T, [element(N, T)|L], N+1).
